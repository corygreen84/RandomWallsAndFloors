...
public class Level_Generator : MonoBehaviour
{
    ...
    void startFromBoxedIn(int leftOffAt)
    {
        Vector3 randomLocation = listOfPositions[UnityEngine.Random.Range(0, listOfPositions.Count - 1)];

        listOfPositions.Add(randomLocation);

        int spacesLeftToFill = levelDepth - leftOffAt;
        for (int h = 0; h < spacesLeftToFill; h++)
        {
            try
            {
                pickRandomSpotAndPlaceTileDown();
    ...
    void pickRandomSpotAndPlaceTileDown(bool cameFromBoxedIn) {
